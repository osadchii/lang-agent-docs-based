...
- **@tanstack/react-query** - серверное состояние, кэш и инвалидация запросов
- **Axios** - HTTP клиент
...
- Централизованный `httpClient` с Axios-интерсепторами (JWT + автоматический refresh через initData).
- React Query QueryClient с едиными политиками кэша/повторов и хуками для историй чатов и профилей.
- Глобальный Error Boundary и компоненты `QueryState` для единообразных заглушек загрузки и ошибок.
