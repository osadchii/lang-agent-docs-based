  ...
  const loadHistory = useCallback(
    async (options?: { offset?: number; append?: boolean }) => {
      ...
      try {
        ...
        setMessages((prev) => (append ? [...response.messages, ...prev] : response.messages));
  ...
  if (authStatus === 'error') {
    return (
      <div className="home-page error-state">
        ...
        <p>{authError}</p>
        <p className="hint">
          Убедитесь, что Mini App открыта внутри Telegram и попробуйте перезапустить приложение.
        </p>
      </div>
    );
  }
      ...
      <header className="home-header">
        <div>
          ...
          <p className="subtitle">
            Привет, {username}! Задай вопрос по языку — мы ответим и сохраним историю диалога.
          </p>
        </div>
      ...
      <section className="chat-panel">
        ...
        <form className="chat-form" onSubmit={handleSubmit}>
          ...
          <textarea
            ...
            rows={3}
          />
          <button type="submit" disabled={!isAuthReady || isSending || !messageText.trim()}>
