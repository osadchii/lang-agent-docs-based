...
| `BACKUP_DIR` | нет | Каталог на сервере для `daily/weekly/monthly` снимков | `/var/backups/postgres` |
| `BACKUP_PREFIX` | нет | Префикс имени архива (`<prefix>_YYYYMMDD_HHMMSSZ.backup.gz`) | `lang-agent` |
| `BACKUP_RETENTION_DAILY_DAYS` | нет | Сколько дней держим ежедневные бэкапы | `30` |
| `BACKUP_RETENTION_WEEKLY_DAYS` | нет | Retention для `weekly` снимков | `90` |
| `BACKUP_RETENTION_MONTHLY_DAYS` | нет | Retention для `monthly` снимков | `365` |
| `BACKUP_WEEKLY_DAY` | нет | День недели (1=Пн .. 7=Вс), когда копируем архив в `weekly` | `7` |
| `BACKUP_MONTHLY_DAY` | нет | День месяца, когда делаем `monthly` | `01` |
| `BACKUP_ENABLE_RCLONE` | нет | Включает загрузку архива в object storage через `rclone` | `false` |
| `BACKUP_REMOTE` | нет | Имя `rclone` remote (обязательно при `BACKUP_ENABLE_RCLONE=true`) | - |
| `BACKUP_REMOTE_PATH` | нет | Папка внутри remote (`backups/<prefix>/<YYYY>/<MM>`) | - |
...
- `scripts/backup.sh` — единый скрипт, который считывает `.env`, делает `pg_dump` (custom format) и складывает архив в `BACKUP_DIR/daily`. По воскресеньям файл копируется в `weekly`, в первый день месяца — в `monthly`, retention управляется переменными `BACKUP_RETENTION_*`.
- Для локальной проверки достаточно выполнить `ENV_FILE=.env BACKUP_DIR=./var/backups ./scripts/backup.sh`. Cron-пример для продакшена: `0 3 * * * cd /opt/lang-agent && ENV_FILE=/opt/lang-agent/.env ./scripts/backup.sh >> /var/log/lang-agent/backup.log 2>&1`.
- `scripts/restore.sh` восстанавливает `.backup`/`.backup.gz` в указанную БД. Пример: `ENV_FILE=.env RESTORE_DROP_EXISTING=true ./scripts/restore.sh var/backups/daily/lang-agent_2025-01-08_030000Z.backup.gz lang_agent_restore`.
- Детализированная инструкция и чек-лист тестирования описаны в `docs/backend-database.md` (раздел «Backup и восстановление») и `docs/deployment.md` (разделы «Резервные копии» и «Восстановление»).
