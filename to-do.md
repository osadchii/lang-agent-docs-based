...
- [x] Alembic: инициализация, базовый `env.py`, `alembic.ini`.
- [x] Миграция 000: функция-триггер `update_updated_at_column()` (общая для всех таблиц).
- [x] Миграция 001: `users` (+ индексы, триггеры) как в спецификации.
- [x] Миграция 002: `language_profiles` (+ ограничения и уникальность активного профиля).
- [x] Миграция 003: `decks` (+ unique active per profile, счетчики).
- [x] Миграция 004: `cards` (все поля SRS, индексы, soft delete).
- [x] Миграция 005: `topics` (личные/групповые, активная тема, soft delete).
- [x] Миграция 006: `conversation_history` (хранение истории диалога с токенами).
- [x] Миграция 007: `exercise_history` (результаты, время, подсказки).
- [x] Миграция 008: `groups`, `group_members`, `group_materials` (если выделено).
- [x] Миграция 009: `subscriptions` (stripe ids, статусы, периоды).
- [x] Денормализация: триггеры обновления счетчиков для колод при изменении карточек.
...
- [x] Redis: инициализация клиента, пространство ключей.
- [x] Пер-пользовательские лимиты: сообщения LLM/день, упражнения/день, карточки (free/premium).
- [x] Глобальные лимиты: per-IP и per-user по `docs/backend-api.md` (заголовки `X-RateLimit-*`).
- [x] Ошибки при превышении лимитов (формат ответа из спецификации).
