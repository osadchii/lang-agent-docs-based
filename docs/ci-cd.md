...
rontend-build — зависит от

rontend-quality, запускается только при push в main. Job собирает Docker-образ по

rontend/Dockerfile (docker/build-push-action + VITE_API_BASE_URL/VITE_ENVIRONMENT) и пушит его в GHCR (ghcr.io/osadchii/lang-agent-docs-based/frontend).
...
- копирует compose/infra и тянет образы через docker compose pull,
...
rontend-build (push в main) собирает



rontend/Dockerfile с аргументами VITE_API_BASE_URL, VITE_ENVIRONMENT и пушит образ ghcr.io/osadchii/lang-agent-docs-based/frontend.
   ...
   - Полученный образ тянет job deploy через docker compose pull frontend.
