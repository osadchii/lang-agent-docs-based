...
3. **Job `deploy`** (только `push` в `main`) открывает SSH-сессию на сервер с помощью `SSH_PRIVATE_KEY_LANG_AGENT`, синхронизирует `docker-compose.yml` в `/opt/lang-agent`, логинится в GHCR через `GHCR_USERNAME/GHCR_TOKEN`, затем выполняет `docker compose pull` и `docker compose up -d --remove-orphans`.
